...
describe('MatrixCommon', () => {
    ...
    it('should edit and reset techniques', inject([MatrixCommon], (service: MatrixCommon) => {
        let technique_list: Technique[] = [];
        let tactic_list: Tactic[] = [];
        let vm1 = service.viewModelsService.newViewModel("test1","enterprise-attack-13");
        let st1 = new Technique(subtechniqueSDO1,[],null);
        ...
        tactic_list.push(tactic1);
        vm1.selectAllTechniques();
        expect(vm1.getEditingCommonValue("score")).toEqual('3');
        expect(vm1.getEditingCommonValue("comment")).toEqual('');
        ...
        vm1.selectAnnotated();
        vm1.resetSelectedTechniques();
        expect(vm1.getTechniqueVM_id('T1592^reconnaissance')['score']).toEqual('');
        vm1.selectAllTechniques();
        let m2 = new Metadata();
        m2.name = "test1";
        m2.value = "t1";
        vm1.editSelectedTechniqueValues("metadata",[m2]);
        expect(tvm_1.metadata).toEqual([m2]);
        vm1.editSelectedTechniques("score", '8');
        expect(tvm_1.score).toEqual('8');
    }));

    it('should select and unselect technique across tactics', inject([MatrixCommon], (service: MatrixCommon) => {
        let technique_list: Technique[] = [];
        let tactic_list: Tactic[] = [];
        let vm1 = service.viewModelsService.newViewModel("test1","enterprise-attack-13");
        let st1 = new Technique(subtechniqueSDO1,[],null);
